...
%%
function [t,y,Y_delta] = SDIRK_general_clipped(t0, tf, h, y0, ode_func, sdirk_method, jacobian )
...
opt.MaxFunctionEvaluationss = 1000;
...
for i = 2:length(t)
    ...
    for istage = 1:s
        ...
        % Y_delta(:,s * (i-2) + istage) = norm(Y(:,istage) - Y_clip(:,istage)) / norm(Y(:,istage));
